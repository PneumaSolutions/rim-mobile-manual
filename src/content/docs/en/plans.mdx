...
Remote Incident Manager is useful in a wide array of scenarios of varying frequency. Thus, our multi-tier plan structure makes every reasonable attempt to meet you where you are. Additionally, remember that RIM is always free for the person receiving help.

### Free Tier

Anyone can use RIM to assist another user for up to 30 minutes a day. These minutes do not need to be used in one sitting, and a usage meter will keep tabs on how many minutes you have on any given day. Throughout your 30 minutes, you may connect to any number of people, over any number of interactive sessions. After each session, RIM will display a popup reminder with your remaining time, as well as a "Sign up for paid RIM services" button.  
<MacOS>
In some cases, pressing escape out of this popup may result in an empty screen in which VoiceOver will announce that Remote Incident Manager has no windows. Should this occur, simply *Command+Tab* to any application. Be careful not to hit Command+Q, otherwise you will not be able to relaunch RIM with the *Option+Shift+Backspace* keyboard command until you manually relaunch RIM.
</MacOS>
...
These packages allow you to have a designated amount of hours on your account. These hours will last for a year from the date of their initial use, or a year from the date of purchase if not used.
...
#### Personal Weekend Plan

The RIM Personal Weekend Support subscription is designed for people who want to provide remote support for family, friends, or community members on the weekend. It provides unlimited RIM connection time for keyword sessions on Saturday and Sunday, in addition to the 30 minutes per day on weekdays provided by the free tier. Note that this subscription does not include unattended access.
...
If you purchase multiple prepaid items such as passes or hour packages, you will receive a separate activation link for each item. This means that, for example, two separate passes can be activated on two separate accounts. This is useful in situations where you wish to gift one of the items of a larger purchase.