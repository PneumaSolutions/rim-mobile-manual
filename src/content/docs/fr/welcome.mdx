...
Remote Incident Manager est véritablement une première en son genre. Il s'agit de la première solution d'assistance à distance *entièrement accessible* qui peut être utilisée par les professionnels de l'assistance et les utilisateurs finaux, qu'ils soient aveugles ou voyants. Ainsi, RIM peut remplacer entièrement une solution existante, car il offre tout ce que l'on peut attendre d'une solution de support à distance sécurisée, en plus d'une accessibilité totale.
#### Facile à installer et à déployer
Dans une situation individuelle, l'installation ne pourrait être plus simple. Une installation instantanée en un clic garantit que le processus de configuration de l'utilisateur final pour recevoir de l'assistance sera rapide et sans douleur. De plus, le processus d'installation lance RIM dès qu'il est terminé, de sorte que l'utilisateur est immédiatement prêt à se connecter.
Les techniciens peuvent facilement gérer leurs machines via une interface web conviviale. Les utilisateurs en entreprise peuvent utiliser ce tableau de bord pour déployer RIM sur les machines de leur lieu de travail. Des installateurs prêts à l'emploi peuvent être envoyés aux utilisateurs finaux en quelques secondes. Le programme d'installation lui-même peut être personnalisé par le biais d'arguments de ligne de commande qui peuvent à leur tour être utilisés dans un déploiement automatisé.
#### Session partagée
...
Fini le temps où il fallait passer un appel téléphonique ou une conférence web en même temps que la session à distance. RIM offre une communication vocale claire et à faible latence au sein même de votre session.
...
* Remote Incident Manager étant multiplateforme, de nombreuses sections de cette documentation adapteront leur contenu au système d'exploitation que vous utilisez.
    ...
    * Si la reconnaissance de la plateforme échoue ou si vous lisez la documentation sur un appareil différent, tel qu'un téléphone ou une tablette, le manuel reprendra la version Windows. Vous trouverez des boutons en haut et en bas de la page qui peuvent être utilisés pour afficher le contenu pour la plateforme de votre choix.
* En changeant de langue, vous accéderez à la section "Bienvenue" du manuel dans la langue choisie.
* Les connexions RIM se font entre un ordinateur cible et un contrôleur.
